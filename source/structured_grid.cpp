...
void structured_grid::init_area_vectors() {
    ...
    for (auto n = 0u; n < num_iface(); ++n) {
        ...
        iface_area_vectors.push_back({ -(v1[1] - v0[1]), v1[0] - v0[0] });
    }
    ...
    for (auto n = 0u; n < num_jface(); ++n) {
        ...
        jface_area_vectors.push_back({ -(v1[1] - v0[1]), v1[0] - v0[0] });
    }
}
...
void structured_grid::init_cell_volumes() {
    ...
    for (auto n = 0u; n < num_cell(); ++n) {
        ...
        cell_volumes.push_back(0.5 * (cross(v1 - v0, v3 - v0) + cross(v3 - v2, v1 - v2)));
    }
}
